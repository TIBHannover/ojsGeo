{**
 * templates/frontend/objects/issue_timeline.tpl
 *
 * Copyright (c) 2026 KOMET project, OPTIMETA project, Daniel Nüst, Tom Niers
 * Distributed under the GNU GPL v3. For full terms see the file LICENSE.
 *
 * @brief Timeline strip showing every article in the issue (issue #74). Injected
 *        from issue_map.tpl directly below the issue map. Shares the per-article
 *        hidden inputs that issue_details.tpl already exposes for the map itself.
 *}

<div id="geoMetadata_issueTimeline" class="geoMetadata_timelineSection">
    <p class="geoMetadata_timelineCollapse">
        <a href="#" class="geoMetadata_timelineCollapseLink"
           ...
           aria-controls="geoMetadata_issueTimelineBody">
    ...
    <div id="geoMetadata_issueTimelineBody" class="geoMetadata_timelineBody"{if $geoMetadata_timelineCollapsedByDefault} style="display:none"{/if}>
        <h3>{translate key="plugins.generic.geoMetadata.timeline.heading"}</h3>
        ...
        {/if}
        <div id="gm-issue-timelinediv" class="geoMetadata_timelineCanvas" style="height: {$geoMetadata_timelineHeight|escape:'html'}px;"></div>
    </div>
</div>
